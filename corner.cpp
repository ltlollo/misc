...
constexpr unsigned w_size{2}, n_features{1500};
constexpr double ets{5000};

struct feature {
    struct pt { unsigned x, y;} p;
    double e;
};
...
int main(int argc, char *argv[]) {
    ...
    for (unsigned i{1}; i < h-w_size-1; ++i) {
        for (unsigned j{1}; j < w-w_size-1; ++j) {
    ...
    std::sort(res.begin(), res.end(), [](const auto& f, const auto& s) {
        return f.e > s.e;
    });
    for_each(res.begin(), res.size() > n_features ?
                 res.begin()+n_features : res.end(), [&](const auto& fe) {
