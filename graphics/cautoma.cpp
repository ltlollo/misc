// gpp cautoma.cpp $(pkg-config --libs libpng)
...
inline px_t
next(px_t *curr, const rules_t rules) {
    px_t ret;
    unsigned r = 0, g = 0, b = 0;
    if (red) {
        r = (((curr - 1)->red & 1) << 2) | ((curr->red & 1) << 1) |
            ((curr + 1)->red & 1);
        if (((rules >> 0) >> r) & 1) {
            ret.red = 255;
        }
    ...
    if (green) {
        g = (((curr - 1)->green & 1) << 2) | ((curr->green & 1) << 1) |
            ((curr + 1)->green & 1);
        if (((rules >> 8) >> g) & 1) {
            ret.green = 255;
        }
    ...
    if (blue) {
        b = (((curr - 1)->blue & 1) << 2) | ((curr->blue & 1) << 1) |
            ((curr + 1)->blue & 1);
        if (((rules >> 16) >> b) & 1) {
            ret.blue = 255;
        }
...
inline void
wrap_line(img_t &img, const size_t r) {
    img[r][0] = img[r][img.get_width() - 2];
    img[r][img.get_width() - 1] = img[r][1];
}

void
populate(img_t &img, const uint32_t rule) {
...
template <typename T>
auto
new_img(size_t x, size_t y, T &&lambda) {
...
auto
random_start(size_t x, size_t y) {
